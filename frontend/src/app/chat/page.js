...
import { MessageSquare, ShieldAlert, SkipForward, X, Send, Video, PhoneOff, Columns } from 'lucide-react';
...
export default function ChatPage() {
  ...
  return (
    <div className="fixed inset-0 w-full h-full bg-black overflow-hidden flex flex-col">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 pb-[calc(24px+env(safe-area-inset-bottom))] pt-10 px-4 bg-gradient-to-t from-black/80 to-transparent flex justify-center pointer-events-none">
        <div className="glass p-2 sm:p-3 rounded-full flex items-center gap-3 sm:gap-4 pointer-events-auto shadow-2xl border border-white/10">
          
          <button className={`w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded-full flex items-center justify-center transition-all ${isSplitScreen ? 'bg-white text-black shadow-lg shadow-white/20 scale-105' : 'bg-white/10 hover:bg-white/20 text-white'}`} onClick={() => setIsSplitScreen(!isSplitScreen)}>
            <Columns className="w-5 h-5 sm:w-6 sm:h-6" />
          </button>

          <button className={`w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded-full flex items-center justify-center transition-all relative ${chatOpen ? 'bg-white text-black shadow-lg shadow-white/20 scale-105' : 'bg-white/10 hover:bg-white/20 text-white'}`} onClick={() => setChatOpen(!chatOpen)}>
            <MessageSquare className="w-5 h-5 sm:w-6 sm:h-6" />
            {!chatOpen && messages.filter(m => !m.isSystem && m.sender !== 'me').length > 0 && (
              <span className="absolute top-0 right-0 w-3.5 h-3.5 bg-blue-500 rounded-full border-2 border-transparent shadow-[0_0_10px_rgba(59,130,246,0.8)]" />
            )}
          </button>

          <div className="w-[1px] h-8 bg-white/10 mx-1 sm:mx-2 rounded-full" />

          <button className="w-12 h-12 sm:w-14 sm:h-14 flex-shrink-0 rounded-full bg-red-500/10 hover:bg-red-500 flex items-center justify-center text-red-500 hover:text-white transition-all shadow-lg hover:shadow-red-500/30" onClick={endCall}>
            <PhoneOff className="w-5 h-5 sm:w-6 sm:h-6" />
          </button>

          <button className="w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 rounded-full bg-gradient-to-tr from-blue-600 to-blue-400 hover:brightness-110 flex items-center justify-center text-white shadow-xl shadow-blue-500/40 transition-all hover:scale-105 active:scale-95" onClick={partner || isMatching ? skipChat : startChat} disabled={!isMediaReady}>
